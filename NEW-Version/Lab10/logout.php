...
if (isset($data['do_login'])) {
    ...
    $cust_password = mysqli_fetch_array(mysqli_query($connect, "SELECT user_password FROM users WHERE user_login = '$cust_name'"));
    $count_name = mysqli_fetch_array(mysqli_query($connect, "SELECT COUNT(*) FROM users WHERE user_login = '$cust_name'"));
    if ($count_name[0] == 1) {
