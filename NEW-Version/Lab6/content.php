...
<div class="main">
    ...
    <div>
        <ul class="jump">
            <li><a href="#Метка1">Что умеет Яndex</a></li>
            <li><a href="#Метка2">Какие варианты Яndex предлагаются</a> </li>
            <li><a href="#Метка3"> Зачем нужны морфология и синтаксис</a></li>
            <li><a href="#Метка4">Развитие системы Яndex, прошлое и будущее</a></li>
        </ul>
    </div>
    ...
    <h2 id="Метка1" style="font-weight: bold;">Что умеет Яndex</h2>
    <p>Программные продукты серии Яndex (Языковый index) - набор средств полнотекстовой индексации и поиска в
        ...
        с учетом логических операторов.<br>
        ...
        Идет индексирование по всем словам, стоп-слова определяются статистически.<br>
        2. Поиск</p>
    <p style="margin-bottom: 0px">Язык запросов: скобки; логическое И (в пределах одного абзаца); логическое
        ИЛИ;
        оператор И НЕ
        (в
        пределах
        одного абзаца); близость - расстояние в словах.
        Список выданных документов упорядочивается по релевантности - по количеству найденных слов. В каждом
        документе выделяются (подсвечиваются) найденные слова. <br>
        3. Работа с языком
        ...
        Алгоритм морфологического разбора умеет корректно обрабатывать и слова, не найденные в словаре. <br>
        Морфология работает для русского и английского языка. Также индексируются числа. </p>
    <h2 id="Метка2" style="margin-top: 0px;text-align: center; font-weight: bold;">Какие варианты Яndex
        предлагаются</h2>
    <p style="margin-bottom:0px">Яndex-Site <br>
        Средство полнотекстовой индексации и поиска по страницам собственного Web-сервера с учетом морфологии
        ...
        Система состоит из следующих частей: </p>
    <p class="radString">
        Индексатор- проходит по всем HTML-документам, лежащим на Web-site'е, и
        создает
        индекс. Индекс дает
        возможность быстро проводить поиск по заиндексированным документам. Предполагается, что эта
        программа
        запускается регулярно, например каждой ночью, чтобы индекс всегда отражал текущее состояние
        Web-site'а.</p>
    <p class="radString">Поисковый сервер - постоянно работающая программа, выполняющая запросы на проведение
        поиска и выделение
        найденных слов в найденных документах (разметки). </p>
    <p class="radString">CGI-скрипт - передает запросы поисковому серверу, получает от него ответ и передает
        этот
        ответ
        пользователю. </p>
    <p class="radString">Морфологический сервер - отвечает на запросы индексатора и поискового сервера. Он
        "знает"
        морфологию
        русского и английского языка</p>
    <h2>Яndex-Dict</h2>
    <p>Задача словарного сервера Яndex-Dict - понимание и расширение запроса пользователя и передача
        ...
        Если слово не существует в словаре ни как имя нарицательное, ни как собственное, то словарный сервер на
    ...
    <h2>Яndex-CD ROM </h2>
    <p>
        Новое приложение, которое позволит создателям компьютерных CD ROM'ов, использующих формат html и браузер
    ...
    <p class="reset">Библиотека полнотекстовой индексации и поиска включает следующие функции: <br>
    ...
    <h2 id="Метка3" style="text-align: center; font-weight: bold;">Зачем нужны морфология и синтаксис</h2>
    <p style="margin-bottom: 0px;">
        Распространено мнение, что шаблон "*", означающий любой набор символов, достаточен для поиска в русских
        ...
        избирательность.
        Случаи, когда шаблон не спасает: </p>
    <p class="radString"> Слова, у которых в разных формах меняется основа (супплетивные формы): идти-шел,
        плохо-хуже, я-меня,
        человек-люди, ребенок-дети </p>
    <p class="radString">Слова с большим количеством словоформ. Привести список всех словоформ русского глагола
        (с
        причастными и
        деепричастными формами - до 250 различных форм) человеку, не имеющему лингвистического образования,
        очень трудно. Понять, все ли словоформы из этого списка "накрываются" шаблоном "*" - ещё труднее.</p>
    <p class="radString">Слова с беглыми гласными и чередованиями. В словаре на 90000 слов, используемом в нашей
        системе,
        примерно четверть слов имеет чередования, которые не позволяют найти слово по шаблонам: (искать-ищу,
        окно-окон, расти-рос: запрос и* или ок* или р* даст много мусора) </p>
    <p class="radString">Короткие (три-четыре буквы) слова: дом, хор, ... . Во всех языках, в том числе и в
        русском, имеется
        общая закономерность: чем чаще слова используются, тем они короче. Применение шаблона "*" в коротких
        словах приводит к большому количеству ненужных ссылок в списке найденных документов.
    </p>
    <h2 id="Метка4" style="font-weight: bold;">Развитие системы Яndex, прошлое и будущее</h2>
    <p style="margin-bottom: 0px;">
        Лингвистическим отделением компании CompTek созданы три информационно-поисковых системы - Справочник
        "Международная Классификация Изобретений", 4 и 5 редакция, а также "Классификатор Товаров и Услуг".
        Общими чертами этих систем являлось:</p>
    <ul class="reset">
    ...
    <p>Первым приложением проекта Яndex стала разработанная в 1994-1995 годах Информационно-поисковая система,
        ...
        (www.comptek.ru/arcadia/yand_rbible.html).
        Готовится к выпуску первый CD-ROM из серии "Электронное научное издание", посвященного жизни и
        творчеству А.С. Грибоедова. Проект "Электронное научное издание" осуществляет НТЦ "ИнформРегистр" и ИМЛИ
        им. М.Горького РАН при поддержке Роскоминформа с 1995 года до настоящего времени. Проект направлен на
        ...
        (Яndex), так и возможности иерархическое гипертекстовое представление текстов.
    </p>
</div>
</div>
</div>
